...
import { useCreateProgram } from "@/lib/api";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "@/hooks/use-toast";

const AddProgramDetail: React.FC = () => {
  const navigate = useNavigate();
  const { trigger: createProgram } = useCreateProgram();
  const [program, setProgram] = useState<Omit<IProgram, "id">>({
    title: "",
    shortDescription: "",
    fullDescription: "",
    startDate: "",
  ...
  const handleFieldChange = (field: keyof IProgram, value: any) => {
    setProgram((prevProgram) => ({ ...prevProgram, [field]: value }));
  };
  ...
  const handleTicketChange = (index: number, value: string) => {
    setProgram((prevProgram) => {
      // Split the ticket types string into an array for easy modification, then join back to a string
      const ticketTypesArray = prevProgram.ticketTypes.split(",");
      ticketTypesArray[index] = value; // Update the specific ticket type by index
      return { ...prevProgram, ticketTypes: ticketTypesArray.join(",") }; // Join the array back to a string
    });
  };
  ...
  const handleAddTicket = () => {
    setProgram((prevProgram) => ({
      ...prevProgram,
      ticketTypes: prevProgram.ticketTypes + ",", // Append an empty ticket type to the string
    }));
  };
  ...
  const handleRemoveTicket = (index: number) => {
    setProgram((prevProgram) => {
      const ticketTypesArray = prevProgram.ticketTypes.split(",");
      ticketTypesArray.splice(index, 1); // Remove the ticket at the given index
      return { ...prevProgram, ticketTypes: ticketTypesArray.join(",") }; // Rejoin the array into a string
    });
  };

  const handleMetaKeywordsChange = (value: string) => {
    const keywords = value.split(",").map((keyword) => keyword.trim());
    setProgram((prevProgram) => ({ ...prevProgram, metaKeywords: keywords }));
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({
        ...
        description: "The new program has been successfully created.",
      });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {program.ticketTypes.split(",").map((ticket, index) => (
                    <div key={index} className="flex items-start space-x-2">
                      ...
                      <input
                        type="text"
                        value={ticket} // Using the string-based ticket type here
                        onChange={
                          (e) => handleTicketChange(index, e.target.value) // Updating the ticket string
                        }
